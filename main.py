...
async def send_to_plate(user_message: str) -> str:
    """Send to n8n plate webhook and read reply properly (handles lists)."""
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.post(N8N_PLATE_URL, json={"message": user_message})
            log.info(f"🍽️ Plate response raw: {r.text[:500]}")
            if r.status_code != 200:
                return f"Sorry, n8n returned {r.status_code}."
            ...
            if isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict):
                reply = data[0].get("reply") or data[0].get("message") or data[0].get("text")
            elif isinstance(data, dict):
                reply = data.get("reply") or data.get("message") or data.get("text") or data.get("output")
